...
from .models import Product, CartItem, Cart, UserDetails, Order
...
class CartView(View):
    def get(self, request):
        ...
        return render(request, "myapp/cart.html", {"is_loggedin": request.user.is_authenticated, 'cart_count': cart_count, 'cart_items': cart_items, "total_cost": total_cost, "user_details":user_details })
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                form = ContactForm()
                cart_count = self.get_cart_count(request.user)
                return render(request, "myapp/contact.html", {'form': form, 'message':"Message Sucessfully sent!", "is_loggedin": request.user.is_authenticated, "is_active": 'contact', 'cart_count': cart_count})
